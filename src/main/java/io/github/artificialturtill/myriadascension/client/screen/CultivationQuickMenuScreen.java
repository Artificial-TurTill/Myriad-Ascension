...
public final class CultivationQuickMenuScreen extends Screen {
    ...
    public void renderBackground(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        // Keep the world visible and unblurred behind the overlay.
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (cell >= 0) {
            activateCell(cell, button == GLFW.GLFW_MOUSE_BUTTON_RIGHT ? -1 : 1);
            return true;
        }
    ...
    private void renderCell(GuiGraphics graphics, int index, int x, int y, int w) {
        ...
        graphics.drawCenteredString(font, Component.literal(title), x + w / 2, y + 13, TEXT);

        if (index == 4) {
            graphics.drawCenteredString(
                    font,
                    Component.literal("Future controls"),
                    x + w / 2,
                    y + 34,
                    MUTED);
            return;
    ...
    private void drawFittedCentered(
        ...
        if (font.width(text) <= maxWidth) {
            graphics.drawCenteredString(font, Component.literal(text), centerX, y, color);
            return;
        }
    ...
    private int cellX(int index) {
        return panelLeft() + (index % COLUMNS) * (cellWidth() + GAP);
    }

    private int cellY(int index) {
        return panelTop() + (index / COLUMNS) * (CELL_HEIGHT + GAP);
    }
}
