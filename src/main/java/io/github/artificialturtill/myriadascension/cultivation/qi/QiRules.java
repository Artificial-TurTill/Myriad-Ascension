...
public final class QiRules {
    ...
    public static final double ACTIVE_GATHER_QI_FRACTION_PER_CONTROL_PULSE = 0.0045D;
    ...
    public static double activeGatherPerControlPulse(double maximumQi, int meditationLevel) {
        double base = Math.max(
                ACTIVE_GATHER_MIN_PER_CONTROL_PULSE,
                Math.max(0.0D, maximumQi) * ACTIVE_GATHER_QI_FRACTION_PER_CONTROL_PULSE);
        return base * (1.0D + clampSkillLevel(meditationLevel) * 0.08D);
    }
}
